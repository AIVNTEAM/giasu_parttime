...
export class ListComponent implements OnInit {
  ...
  getUsers(page:number = 1) {
    ...
    this.appService.get('bentimviec', paramsApi).subscribe((res:any) => {
      //if (res.status == 200) {
        console.log(res);
        this.data = res.data;
        this.dsbentimviec = this.data.data;
  ...
  changeSort(field) {
    this.router.navigateByUrl('/admin/bentimviec');
  ...
  delete(user) {
    // this.layoutService.confirmDelete(() => {
      this.appService.post('bentimviec/delete', {id: user.id}).then(response => {
