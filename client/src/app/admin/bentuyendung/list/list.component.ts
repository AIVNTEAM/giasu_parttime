...
export class ListComponent implements OnInit {
  ...
  getUsers(page:number = 1) {
    ...
    this.appService.get('bentuyendung', paramsApi).subscribe((res:any) => {
      //if (res.status == 200) {
        console.log(res);
        this.data = res.data;
        this.dsbentuyendung = this.data.data;
  ...
  changeSort(field) {
    this.router.navigateByUrl('/admin/bentuyendung');
  ...
  delete(user) {
    // this.layoutService.confirmDelete(() => {
      this.appService.post('bentuyendung/delete', {id: user.id}).then(response => {
