...
export class ListComponent implements OnInit {
  ...
  ngOnInit() {
  	...
  	window.scrollTo(0,0);
    this.route.params.subscribe((params:Params) => this.getDanhmucViec(params['page']));
  }

  getDanhmucViec(page:number = 1) {
    ...
    this.appService.get('danhmucviec', paramsApi).subscribe((res:any) => {
      //if (res.status == 200) {
        console.log(res);
        this.data = res.data;
        this.dsdmviec = this.data.data;
  ...
  changeSort(field) {
    this.router.navigateByUrl('/admin/danhmucviec');
  ...
  change()
  {
    this.router.navigateByUrl('/admin/viec');
    this.getDanhmucViec();
  }
  ...
  delete(dmviec) {
    // this.layoutService.confirmDelete(() => {
      this.appService.post('danhmucviec/delete', {id: dmviec.id}).then(response => {
  ...
  changeStatus(id: number, active, model, current_page = 1) {
    this.appService.post('change_status', {'id' :id, 'active': active, 'model': model}).then(res => {
      if (res.status == 200) {
        this.getDanhmucViec(current_page);
      }
    });
  }
