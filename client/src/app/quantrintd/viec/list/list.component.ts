...
export class ListComponent implements OnInit {
  ...
  getViec(page:number = 1) {
    ...
    this.appService.get('viec', paramsApi).subscribe((res:any) => {
      //if (res.status == 200) {
        console.log(res);
        this.data = res.data;
        this.dsviec = this.data.data;
  ...
  changeSort(field) {
    this.router.navigateByUrl('/quantrintd/viec');
  ...
  delete(viec) {
    // this.layoutService.confirmDelete(() => {
      this.appService.post('viec/delete', {id: viec.id}).then(response => {
